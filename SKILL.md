...
Step 2: 分析服务
   ├── Step 2a: 网关模式 → 分析路由配置
   └── Step 2b: 业务服务模式 → 分析Controller/Service
...
**流程关键点**：
- 每次分析完成后必须回到 Step 5 询问
- 只有用户选择"结束探索"后才进入 Step 6
- 支持多次探索，汇总后统一生成图表
...
注意：此时不生成最终文件，仅内部记录，等待探索结束后统一输出
```
...
- 选项2 → 输入具体节点 → Step 2b 深入分析 → 分析完返回 Step 5
- 选项3 → 列出未追踪的服务 → 用户选择 → Step 3 递归追踪 → 追踪完返回 Step 5
- 选项4 → 结束探索 → 进入 Step 6 生成图表
- 选项5 → 输出JSON结果 → 结束

【关键】：每次分析完成后必须回到 Step 5 询问，直到用户选择选项4或5才结束探索！
```
...
4. 调用 drawio skill，根据合并后的JSON生成.drawio文件
5. 输出最终追踪路径摘要
6. 结束
```
...
• edge-gateway → flow-service → rule-engine (未追踪)
...
本次分析中发现但未深入追踪的服务:
• order-service (网关路由，用户选择skip)
• rule-engine (flow-service → Dubbo调用，用户选择skip)

请选择要追踪的服务 (输入服务名或路径，skip全部跳过，quit返回): skip
...
请选择要追踪的服务 (输入服务名或路径，skip全部跳过，quit返回):
请输入: rule-engine

请输入 rule-engine 的代码路径: /projects/rule-engine

──────────────────────────────────────────────────────────
正在追踪 rule-engine... (回到 Step 3)
──────────────────────────────────────────────────────────
找到: RuleEngine.process
  → 规则计算
  → DB: ruleMapper.findByRuleId
...
本次分析发现的路径:
• flow-service → process_task → task-service → notification-service
• notification-service → notification_queue → notify-service
• flow-service → rule-engine (新增)
...
汇总追踪结果:
- 涉及服务: flow-service, task-service, notification-service, notify-service, rule-engine
- 异步表: process_task, notification_queue
- 外部服务调用: HTTP, Dubbo
...
5. **【强制】探索询问**：每次分析完成后必须回到 Step 5 询问，直到用户选择结束探索
6. **探索结束后生成图表**：只有用户选择"结束探索，生成图表"后才进入 Step 6
7. **不支持的调用**：
   - 反射调用
   - 动态代理
   - 运行时生成的代码
...
| 时机 | 询问内容 | 下一步 |
|------|----------|--------|
| 发现外部服务调用 | 是否继续追踪该服务？ | 输入路径→Step 3；skip→记录待追踪；quit→结束 |
| 发现嵌套异步表 | 是否继续分析下游流程？ | 输入路径→Step 3；skip→记录待追踪；quit→结束 |
| 单个服务分析完成 | 是否继续探索？(Step 5) | 继续→回到Step 1/2b/3；结束→Step 6 |
| 用户选择skip的服务 | 在Step 5时列出，询问是否追踪 | 选择→Step 3；不追踪→保留skip状态 |
...
Step 2 (分析)
    ↓
Step 3 (递归追踪，发现新服务时询问)
    ↓
Step 4 (记录结果)
    ↓
Step 5 (探索询问) ←─── 继续探索则回到前面步骤
    ↓
用户选择"结束探索，生成图表"
    ↓
Step 6 (生成流程图)
...
```mermaid
flowchart TB
    Start[开始] --> Step1[Step 1: 判断入口类型]
    Step1 --> Step2a{网关?}
    Step2a -->|是| GW[Step 2a: 分析网关路由]
    Step2a -->|否| Step2b[Step 2b: 分析业务服务]
    
    GW --> AskPath[询问下游服务路径]
    AskPath --> Step2b
    
    Step2b --> Step3[Step 3: 递归追踪]
    Step3 --> FoundExternal{发现外部服务?}
    
    FoundExternal -->|是| AskExternal[询问是否追踪]
    AskExternal -->|输入路径| Step3
    AskExternal -->|skip| RecordSkip[记录待追踪]
    RecordSkip --> Step3
    
    FoundExternal -->|否| FoundAsync{发现异步表?}
    FoundAsync -->|是| AskAsync[询问是否分析下游]
    AskAsync -->|输入路径| Step3
    AskAsync -->|skip| RecordSkip
    
    FoundAsync -->|否| Step4[Step 4: 记录结果]
    Step4 --> Step5[Step 5: 探索询问]
    
    Step5 --> UserChoice{用户选择}
    UserChoice -->|1.新入口| Step1
    UserChoice -->|2.深入节点| Step2b
    UserChoice -->|3.追踪未分析| Step3
    UserChoice -->|4.结束探索| Step6[Step 6: 生成图表]
    UserChoice -->|5.仅JSON| Output[输出JSON]
