...
```
Step 0: 查看上下文 → 调用脚本查看已分析服务 → 确定当前分析目标
Step 1: 解析入口点 → 定位服务代码目录 → 判断网关/业务服务
Step 2: 分析服务 → 网关模式(分析路由) / 业务服务模式(分析代码)
Step 3: 递归追踪 → 发现跨服务调用/异步表 → 询问是否继续
Step 4: 保存结果 → 调用脚本保存JSON → 确保不遗失
Step 5: 探索询问 → 继续探索(回Step0-3) / 结束探索(进Step6)
Step 6: 生成图表 → 调用脚本汇总 → Mermaid生成时序图(默认)
```
...
```mermaid
flowchart TB
    Step0[Step 0: 查看上下文] --> Step1[Step 1: 解析入口]
    Step1 --> Step2{网关?}
    ...
    FoundCall -->|下游服务| CheckConfig{已配置目录?}
...
**区分两类询问**：
- **Step 3 局部询问**：追踪过程中发现跨服务调用/异步表时，针对单个调用点的即时询问（是否追踪该服务？路径是什么？）。这是**可选的**——如果已配置目录可自动跳过。
...
| 发现跨服务调用(HTTP/RPC) | 询问是否继续追踪？ | 已配置目录→直接使用；未配置→询问路径；skip→记录待追踪 |
...
**分析下游时继续识别**：跨服务调用 → 询问是否追踪；嵌套异步表 → 询问是否分析。
...
跨微服务分析过程中，模型可能遗忘之前分析的服务。使用脚本持久化每个服务的分析结果：
...
# 查看所有已分析服务
python ~/.agents/skills/flow-trace/scripts/flow_trace_record.py list

# 查看某个服务详情
python ~/.agents/skills/flow-trace/scripts/flow_trace_record.py get <服务名>
...
python ~/.agents/skills/flow-trace/scripts/flow_trace_record.py save user-service UserController.login '{
  ...
  "calls": [
    {"type": "HTTP", "target": "auth-service", "path": "/api/verify"},
    {"type": "DB", "table": "users"}
  ]
}'
```
...
4. **🔴 强制保存结果**：每次分析完成后**必须**调用脚本保存结果
5. **🔴 强制探索询问**：保存后**必须**展示探索询问菜单
6. **探索后生成图表**：只有用户选择"结束探索"后才进入Step6
7. **异步流程递归**：发现跨服务调用或嵌套异步表必须询问是否继续
